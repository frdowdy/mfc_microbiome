...
```{r Co-occurrence, message = FALSE, warning=FALSE}
...
#filter OTU's to 50% presence in all samples
  ...
  #run co-occurence. Taxon can be excluded and identified later if desired.
  biom_netw_bristles_pearson=cooccurrence(data = biom_fil_bristles,taxon = biom$taxon,type="pearson",cor=.6,pval=.05)
  biom_netw_bristles_spearman=cooccurrence(data = biom_fil_bristles,taxon = biom$taxon,type="sp",cor=.6,pval=.05)
  #try plotting the data
  # plot.igraph(biom_netw$netw) # standard plot function for igraph package
  # tkplot(biom_netw$netw) #interactive plot viewer
  # rglplot(biom_netw$netw) #experimental 3d plot viewer
  ...
  # Visualize
  set.seed(101)
  (network_plot_bristles_pearson <- ggnet2(biom_netw_bristles_pearson$netw, size = 6, color = "tomato", label=T, label.size=2))
  (network_plot_bristles_spearman <- ggnet2(biom_netw_bristles_spearman$netw, size = 6, color = "tomato", label=T, label.size=2))
  (network_plot_bristles_subtract_ps <- ggnet2(biom_netw_bristles_pearson$netw-biom_netw_bristles_spearman$netw, size = 6, color = "tomato", label=T, label.size=2))
    
  # Try labeling OTUs
  # Try coloring according to sample
  # Try iterating different cutoff points
```
# Community Detection
...
```{r anova on these Box Cox transformed relative abundances by sample}
# Data
